...
class WALKERS(object):
    ...
    def _move_one(self, step_size):
        ...
        for iw in range(self.nwalkers):
            if self.status[iw] == 1:
                ...
                new_pos[iw, index[0]:index[1]
                        ] += self._random(step_size, (self.ndim,))
        return new_pos
    ...
    def move_dmc(self, step_size, drift, ielec):
        ...
        new_pos = self.pos
        new_pos[:, self.ndim*ielec:(self.ndim+1)*ielec] += step_size * \
            drift[:, self.ndim*ielec:(self.ndim+1)*ielec]
        new_pos[:, self.ndim*ielec:(self.ndim+1)*ielec] += np.random.normal(
            loc=0.0, scale=step_size, size=self.ndim)
        return new_pos
